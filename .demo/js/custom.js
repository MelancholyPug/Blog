$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    for (i = 0; i < oData.length; i++) {
      ...
      else { iPrepare = 5; iAllowDelay = 15; }  //2020年改不限身分節次一律允許遲到15分鐘
      //計算其餘相關時間
      ...
      if (!oTimeNow.isAfter(oTimeStop.clone().add(iNextInterval, "seconds"))) { break; }
    }

    //計算現在是否為最後一節
    bIsLastClass = iClassNumber === oData.length;
    //顯示所有已經計算過的時間
    ...
    if (oTimeNow.isAfter(oTimePrepare)) {
      ...
      if (bIsLastClass && oTimeNow.isAfter(oTimeStop)) {
    ...
    //時間有經過就核勾
    if (oTimeNow.isAfter(oTimePrepare)) { $("#uTimePrepare").next().html(cChecked); }
    else { $("#uTimePrepare").next().html(cNotAvailable); }
