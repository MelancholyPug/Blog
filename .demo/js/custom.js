$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    for (i = 0; i < oData.length; i++) {
      ...
      //是否為兩小時等級考試
      if (!oData[i].bIs2HR)
      { oTimeStop = oTimeStart.clone().add(1, "hours"); }
      else
    ...
    if (oTimeNow.isAfter(oTimeLogin))
    { $("#uTimeLogin").next().html(cChecked); }
    else
    { $("#uTimeLogin").next().html(cNotAvailable); }

    if (oTimeNow.isAfter(oTimeStart))
    { $("#uTimeStart").next().html(cChecked); }
    else
    { $("#uTimeStart").next().html(cNotAvailable); }

    if (oTimeNow.isAfter(oTimeDisable))
    ...
    { $("#uTimeFinish").next().html(cNotAvailable); }

    if (oTimeNow.isAfter(oTimeStop))
    { $("#uTimeStop").next().html(cChecked); }
    else
    { $("#uTimeStop").next().html(cNotAvailable); }
